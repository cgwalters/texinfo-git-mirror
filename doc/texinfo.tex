...
\def\everyheadingxxx#1{\oddheadingxxx{#1}\evenheadingxxx{#1}}%
...
\def\everyfootingxxx#1{\oddfootingxxx{#1}\evenfootingxxx{#1}}
...
\def\tocentry{%
  % The following vskip is not necessary, as \entry contains another one.
  % But it doesn't hurt, so why should we alter the appearance of all the
  % manuals?
  \vskip 0pt plus1pt
  \entry
}
...
\def\printdefunline#1\DefunTerm{%
  \begingroup
    ...
    % rendering the following check redundat.  But we don't optimize.
    \checkparencounts
  \endgroup
}
...
% \deffnheader has to be defined by explicitely.
%
...
%
% Prevent hyphenation at `-' chars.
%
\def\defunargs#1{%
  \magicamp
  \sl\hyphenchar\font=0
  #1%
